...
for i, func in enumerate(funcsX[::steps]):
    output += "\\addplot[\n\t\t\t\tdomain = -1:1,\n\t\t\t\tsmooth,\n\t\t\t\tultra thick,\n\t\t\tdotted,\n\t\t\t\tgray!" + str((i + 5) * 100 * steps // len(funcsX)) + "\n\t\t\t] {" + func + "};"
...
for i, func in enumerate(funcsY[::steps]):
    output += "\\addplot[\n\t\t\t\tdomain = -1:1,\n\t\t\t\tsmooth,\n\t\t\t\tultra thick,\n\t\t\tdotted,\n\t\t\t\tgray!" + str((i + 5) * 100 * steps // len(funcsY)) + "\n\t\t\t] {" + func + "};"
...
for i, func in enumerate(funcsZ[::steps]):
    output += "\\addplot[\n\t\t\t\tdomain = -1:1,\n\t\t\t\tsmooth,\n\t\t\t\tultra thick,\n\t\t\tdotted,\n\t\t\t\tgray!" + str((i + 5) * 100 * steps // len(funcsZ)) + "\n\t\t\t] {" + func + "};"
...
for i in range(0, len(funcsX), 5):
	output += "\t\t\t\\addplot3+[no markers, samples=25, samples y=0, smooth, dotted, ultra thick, domain = 0:1, variable =\\t, gray!" + str(i * 100 // len(funcsX)) + "] ({" + funcsX[i].replace("x", "\\t") + "}, {" + funcsY[i].replace("x", "\\t") + "}, {" + funcsZ[i].replace("x", "\\t") + "});\n\n"

output += "\t\t\t\\addplot3+[no markers, samples=25, samples y=0, smooth, solid, ultra thick, domain = 0:1, variable =\\t, purple] ({" + funcsX[-1].replace("x", "\\t") + "}, {" + funcsY[-1].replace("x", "\\t") + "}, {" + funcsZ[-1].replace("x", "\\t") + "});\n\n"
